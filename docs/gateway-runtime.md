...
Skill / MCP：**定义**在 Gateway，**进程**跑在实例旁边。实例按当前账号可见集合成请求头里的工具表。模型密钥一把都不下发到实例（见 §13 和本节末尾那条 `~~公司密钥下发到实例~~ **否**`）。
...
**平台密钥 > 环境变量**，和第 3 节那条「公司不再各自贴 key」是同一件事：供应商只由 `owner` 在
`/platform/credentials` 配，全平台共用。

> 这里一度是「公司密钥 > 平台密钥 > 环境变量」，公司管理员在 `/orgs/:id/credentials` 贴自己
> 那几把、压过平台的。那一层**已经整条撤掉**：路由（列表/详情/POST/PUT/DELETE）、
> `POST /orgs/:id/llm/test`、界面上的「供应商」菜单和整页，以及 `llm.secret` 里公司那一档。
> `credentials` 表和各家存进去的历史数据还在，但没有任何一条路读它或写它——**尤其不许在开机
> 时把它们升成平台密钥**，那是跨租户漏密钥（见 `gateway/src/db.ts` 的
> `liftCompanySettingsToPlatform`）。
>
> 代价是明知道的：平台那把被限流或吊销时，所有公司同时哑，而且没有一家能自救，只能等 owner
> 换掉平台那把。
...
| GET/POST | `/runtime/local-routines/due`、`…/:runId/started\|renew\|finish` | 桌面端里的本地 Bot 凭 `sat_` 领自己账号本地 Bot 的日常任务，四步同 `/worker/routines/*`，流水 machineId 为 `desktop:<accountId>` |
