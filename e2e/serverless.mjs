...
export async function runServerless({ root, gwRoot, test, req, start, waitHttp, assert, log }) {
  ...
  const probe = async (base, label) => {
    ...
    // 桌面端把界面打进了包里，页面源是 satu://localhost：预检要 204 带 allow-origin，别的源一律不给。
    const pre = await fetch(`${base}/runtime/bots`, { method: 'OPTIONS', headers: { origin: 'satu://localhost', 'access-control-request-method': 'GET', 'access-control-request-headers': 'authorization' } })
    assert(pre.status === 204 && pre.headers.get('access-control-allow-origin') === 'satu://localhost', `${label} 桌面源预检 ${pre.status} ${pre.headers.get('access-control-allow-origin')}`)
    assert(String(pre.headers.get('access-control-allow-headers')).includes('authorization'), `${label} 预检没放 authorization`)
    const evil = await fetch(`${base}/runtime/bots`, { method: 'OPTIONS', headers: { origin: 'https://evil.example', 'access-control-request-method': 'GET' } })
    assert(!evil.headers.get('access-control-allow-origin'), `${label} 别的源不该拿到 allow-origin`)
    const withOrigin = await fetch(`${base}/health`, { headers: { origin: 'satu://localhost' } })
    assert(withOrigin.headers.get('access-control-allow-origin') === 'satu://localhost', `${label} 正式请求的响应上也要有 allow-origin`)
