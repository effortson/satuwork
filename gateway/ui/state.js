...
const state = {
   ...
   * 公司管理员侧栏各分类的展开状态。
   *
   * **键没写进来的一律当展开**（render.js 里那句 `!== false`），以后新增分类不用
   * 回这儿补一行。「公司」是唯一一个明写 false 的：那八条是管理员偶尔才去一趟的
   * 后台（席位、审计、账单…），而侧栏上面就是每天要点的 Bot 名单——默认摊开八行，
   * 名单会被挤得只剩两三个，每次进来都得先滚一段才看得见自己的 Bot。
   *
   * 折不折是**这一次会话里的事**，不落盘：展开之后刷新一下又收回去。这条如果哪天
